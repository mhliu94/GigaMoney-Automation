...
The holdings query retries its complete read-only workflow once after a failure. The first failure force-stops and relaunches Gigamoney, then begins again from the landing-page preflight; a second failure is reported unchanged through the existing runner logging path.

Before market, limit, fill-or-kill, cancel-all, and holdings operations begin, the scripts also inspect a screenshot for Gigamoney's dim ad overlay. They check again immediately before order submission and cancellation confirmation. The guard requires a brighter center with dim outer regions and a dark lower screen; a bottom-lit confirmation screen is explicitly excluded. A confirmed ad is dismissed with Android Back and checked again before the operation continues. An ambiguous overlay force-stops and relaunches Gigamoney once, then rechecks the screen; an overlay that remains ambiguous after the restart, or a persistent confirmed ad, stops the operation instead of allowing UI automation to proceed.
